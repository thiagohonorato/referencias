			...
			if ((miniCart.itemGroups != null && miniCart.itemGroups.length != 0)|| (miniCart.donations != null && miniCart.donations.length != 0)) {
				...
				jQuery.each(miniCart.itemGroups, function(){
					...
					jQuery(".mc-content").append(
							...
							'<h4 class="mc-sub-section">'+ itemGroup.salesChannelName + '</h4>'+
							'<div>'
					);
					...
					jQuery.each(itemGroup.items, function(){
						...
						jQuery(".mc-content .mc-section").filter(":last").append(
								...
								'<div id="mc_item-row-id-'+ item.lineItemId + '" callback-data="mc_item-row-id-' + item.lineItemId + "|" + item.partNumber +  "#" + item.productType + "~" + itemTitle + '" class="'+ rowClassName +'">'+
				...
				if(miniCart.donations != null && miniCart.donations.length != 0){
					...
					var donationsHeader = 	'<div class="mc-section">'+				
											...
											'Donations<span id="mc-donation-qty">('+ miniCart.donationQty+')'+ '</span></div><div class="mc-donation-total">US</span>$' + miniCart.donationTotal+ '</div><div class="mc-clearit">&nbsp;</div></div>';
					...
					if(donationsOpened){
						donationsHeader = 	'<div class="mc-section">'+				
											...
											'Donations<span id="mc-donation-qty" style="display:none">('+ miniCart.donationQty+')'+ '</span></div><div class="mc-donation-total" style="display:none">US</span>$' + miniCart.donationTotal+ '</div><div class="mc-clearit">&nbsp;</div></div>';
					}
