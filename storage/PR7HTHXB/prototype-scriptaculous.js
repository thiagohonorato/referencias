	...
	create: function() {
		...
		var behavior = function() {
			if (!this.initialize) {
				...
				var args = (arguments.length == 2 && arguments[1] instanceof Array) ? arguments[1] : Array.prototype.slice.call(arguments, 1);
...
Remote = Behavior.create({
	initialize: function(options) {
		if (this.element.nodeName == "FORM") {
			new Remote.Form(this.element, options)
		} else {
			new Remote.Link(this.element, options)
		}
	}
});
Remote.Base = {
	...
	_makeRequest: function(options) {
		if (options.update) {
			new Ajax.Updater(options.update, options.url, options)
		} else {
...
Remote.Form = Behavior.create(Remote.Base, {
	onclick: function(e) {
		...
		if (["input", "button"].include(sourceElement.nodeName.toLowerCase()) && sourceElement.type == "submit") {
	...
	onsubmit: function() {
		var options = Object.extend({
			...
			parameters: this.element.serialize({
				submit: this._submitButton.name
			})
		}, this.options);
...
Observed = Behavior.create({
	...
	_observeForm: function() {
		return (this.options.frequency) ? new Form.Observer(this.element, this.options.frequency, this.callback) : new Form.EventObserver(this.element, this.callback)
	},
	_observeField: function() {
		return (this.options.frequency) ? new Form.Element.Observer(this.element, this.options.frequency, this.callback) : new Form.Element.EventObserver(this.element, this.callback)
	}
});
